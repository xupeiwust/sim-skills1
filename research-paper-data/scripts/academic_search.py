...
# ///
"""No-MCP scholarly search through public OpenAlex and Crossref APIs.

This is the graceful-degradation path for agents/environments that do not mount
an MCP server or managed provider. It is a single self-contained file hitting
public HTTP APIs with no third-party dependencies. Plain topic searches fall
back to Crossref when OpenAlex is temporarily unavailable.
...
preprints and many repository deposits for discovery. Resolve selected records
through authoritative DOI/arXiv/PMID sources before claiming final metadata.
...
OPENALEX_MAILTO, CROSSREF_MAILTO, or RESEARCH_CONTACT_EMAIL environment variable.
...
# Polite-pool contact; overridden by --mailto or environment variables.
MAILTO = ""
...
def main():
    ...
    # Resolve the polite-pool contact: --mailto > source env vars > default.
    global MAILTO
    MAILTO = (args.mailto or os.environ.get("OPENALEX_MAILTO")
              or os.environ.get("CROSSREF_MAILTO")
              or os.environ.get("RESEARCH_CONTACT_EMAIL") or MAILTO)
